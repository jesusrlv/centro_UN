...
<?php
    ...
    if(move_uploaded_file($_FILES["file1"]["tmp_name"],"../docs/".$id.'_'.$annio.'_'.$categoria.'_'.$subcategoria.'.'.$extension)){
        // echo "$fileName carga completa";     
        
        $ruta = $id . '_' . $annio . '_' . $categoria . '_' . $subcategoria . '.'.$extension;
        ...
        if($resultado){
            ...
            }).then(function(){window.location='../05_manual_contabilidad.php';});  
            </script> 
            ";
        }
        else{
            echo 'No se registró<br>';
            echo $id.'<br>';
            $error = $conn->error;
            echo $error;
